...
export const listProducts: RequestHandler = async (req, res) => {
  ...
  const [rows, count, facets, summary] = await Promise.all([
    pool.query(`SELECT *,(current_stock<=minimum_stock) low_stock,CASE WHEN current_stock=0 THEN 'Out of Stock' WHEN current_stock<=minimum_stock THEN 'Low Stock' ELSE 'Healthy' END stock_health FROM products ${where} ORDER BY updated_at DESC LIMIT $5 OFFSET $6`, [...values,limit,offset]),
    ...
    pool.query(`SELECT COUNT(*)::int total_products,COALESCE(SUM(current_stock*unit_price),0) inventory_value,COUNT(*) FILTER(WHERE current_stock>0 AND current_stock<=minimum_stock)::int low_stock,COUNT(*) FILTER(WHERE current_stock=0)::int out_of_stock FROM products`),
  ]);
...
export const createProduct: RequestHandler = async (req,res) => {
  ...
  const product=await withTransaction(async (db) => {
    const created=(await db.query(`INSERT INTO products(product_name,sku,category,unit_price,current_stock,minimum_stock,warehouse_location,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[d.product_name,d.sku,d.category,d.unit_price,initial,d.minimum_stock,d.warehouse_location,req.user.id])).rows[0];
...
export const updateProduct: RequestHandler = async(req,res) => {
  ...
  const product=await withTransaction(async(db)=>{
    const updated=(await db.query(`UPDATE products SET product_name=$1,sku=$2,category=$3,unit_price=$4,minimum_stock=$5,warehouse_location=$6,updated_at=NOW() WHERE id=$7 RETURNING *`,[d.product_name,d.sku,d.category,d.unit_price,d.minimum_stock,d.warehouse_location,req.params.id])).rows[0];
