...
export const api=async(path,options={})=>{
  ...
  if(!response.ok){if(response.status===401){localStorage.removeItem('orbit_token');localStorage.removeItem('orbit_user');sessionStorage.removeItem('orbit_token');sessionStorage.removeItem('orbit_user');window.dispatchEvent(new Event('orbit:session-expired'));}throw new ApiError(data.error?.message||data.message||'Request failed.',response.status,data.error?.details||data.errors);}
